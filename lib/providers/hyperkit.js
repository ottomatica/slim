...
const ping = require('ping-subnet');
...
class Hyperkit {
    ...
    async macToIP(mac) {
        // for some reason arp strips the leading zero
        mac = mac.split(':').map(v => v.replace(/^0/, '')).join(':');
        let re = new RegExp('.+\\((\\d+.\\d+.\\d+.\\d+)\\) at '+mac+'.+');
        while (true) {
            // ping ips to add them to arp
            await this.pingSubnet();

            let output = child.execSync('arp -a').toString();
    ...
    async pingSubnet() {
        return new Promise(resolve => {
            let pinger = new ping([ '192.168.64.1-192.168.64.255' ]);
            pinger.once('ping:end', () => resolve());
            pinger.ping();
        });
    }
}
