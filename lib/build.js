...
const formatSteps = {
    'vhd': [dockerBuild, dockerExport, moveBoot, buildRootfsImage, makeVhd],
    'raw': [dockerBuild, dockerExport, uncompressKernel, rawExtract, buildRootfsImage, cleanup],
    'iso': [dockerBuild, dockerExport, uncompressKernel, rawExtract, buildRootfsImage, isoBuild, cleanup],
    'qcow2': [dockerBuild, dockerExport, uncompressKernel, rawExtract, buildRootfsImage, isoBuild, qcowBuild, cleanup]
};
...
async function moveBoot(context) {
    info('moving vmlinuz and init to /boot');
...
async function cpioPack(context) {
    ...
    // return;
    let { formatOptions } = context;

    let vmDir = path.join(slimdir, 'slim-vm');
    let output = path.join(slimdir, 'rootfs')
    let zip = formatOptions.zip;
